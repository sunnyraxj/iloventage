...
export function HeaderClient({ categories, settings }: HeaderClientProps) {
  ...
  const getSearchSuggestions = (isMobileView: boolean) => {
    const linkClassName = isMobileView
      ? "flex items-center gap-4 p-2 rounded-md transition-colors duration-200 active:bg-accent"
      : "flex items-center gap-4 p-2 rounded-md transition-all duration-200 hover:bg-accent hover:shadow-md hover:scale-105";
    const imgSizeClass = isMobileView ? "h-12 w-12" : "h-10 w-10";
    ...
    return showSearchResults ? (
      <div>
        ...
        {isSearching ? (
          ...
          <div className={cn("space-y-1", isMobileView && "max-h-[calc(100vh-8rem)] overflow-y-auto")}>
            {searchResults.slice(0, 10).map(product => (
              ...
              >
                <img src={product.variants[0]?.imageUrls[0] || `https://picsum.photos/seed/${product.id}/50/50`} alt={product.name} className={`${imgSizeClass} object-cover rounded-md`} />
  ...
  if (isMobile && isMobileSearchActive) {
      return (
        <div className="fixed inset-0 z-50 bg-background md:hidden">
            <div className="container flex h-16 items-center gap-2 border-b">
                ...
                <form className="w-full" onSubmit={handleSearchSubmit}>
                    <div className="relative">
                        <Input
                            ...
                            className="w-full bg-transparent pl-4 pr-12 h-10 border-0 focus-visible:ring-0 text-base"
                            value={searchQuery}
